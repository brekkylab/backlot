...
CORPUS = [
    {"source_type": "hubspot", "object_type": "companies", "doc_id": "hs-co-acme",
     "title": "Acme Health", "content": "Mid-market healthcare provider, EU data residency required.",
     "properties": {"name": "Acme Health", "domain": "acme-health.com",
                    "industry": "healthcare", "lifecyclestage": "evaluation"}},
    {"source_type": "hubspot", "object_type": "notes", "title": "",
     "content": "Security review scheduled; customer blocked on confirming EU data residency.",
     "properties": {"hs_note_body": "Security review scheduled; customer blocked on confirming "
                                    "EU data residency."},
     "associations": [{"to": "hs-co-acme"}]},
    {"source_type": "hubspot", "object_type": "notes", "title": "",
     "content": "Pricing pushback: wants per-query rather than reserved capacity.",
     "properties": {"hs_note_body": "Pricing pushback: wants per-query rather than reserved "
                                    "capacity."},
     "associations": [{"to": "hs-co-acme"}]},
...
QUESTION = ("Find the healthcare company in the CRM, then read the notes associated with it and the "
...
def build_params(base_url: str, token: str) -> StdioServerParameters:
    ...
    return StdioServerParameters(
        ...
        args=[_BRIDGE, "--source", "hubspot", "--base-url", base_url.rstrip("/"),
              "--token", token])
