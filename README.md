...
| `/github` | GitHub | `search/issues` (`q`: free text + `repo:` `is:` `state:` `type:` `label:` `author:`), `orgs/{org}`, `orgs/{org}/repos`, `user/repos` (the token's own reach), `repos/{o}/{r}`, `.../issues[/{n}]`, `.../issues/{n}/comments`, `.../pulls[/{n}]`, `.../pulls/{n}/reviews`, `.../pulls/{n}/comments`, `.../pulls/{n}/files`, `.../readme`, `.../contents[/{path}]`, `.../git/trees/{ref}`, `.../git/blobs/{sha}`, `.../git/ref/{ref}` (takes the ref as a trailing path, so `heads/release/2026-03` resolves), `.../branches/{branch}`, `.../commits/{sha}`, `.../collaborators`, `.../teams`, `orgs/{org}/teams`. **Media types are honoured**: `Accept: application/vnd.github.raw` on `contents`/`readme`/`git/blobs` returns the file's bytes, and `…diff`/`…patch` on a pull returns a real unified diff / `git am` mbox. The `{owner}` segment is validated against the served org and 404s otherwise, as GitHub does. A pull's changeset (`files`, `additions`/`deletions`/`changed_files`, `patch`) is synthesized deterministically from the repo's own snapshot — it names no file the repo lacks, and the diff applies with real `git` |
