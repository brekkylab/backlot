...
CORPUS = [
    ...
    {"source_type": "hubspot", "object_type": "contacts", "title": "Ava Stone",
     "content": "Ava Stone — VP Platform at Acme Health.", "author_email": "rep@acme.com",
    ...
    {"source_type": "hubspot", "object_type": "deals", "title": "Acme Health — renewal",
     "content": "12-month renewal, EU residency required.", "author_email": "rep@acme.com",
    ...
    {"source_type": "hubspot", "object_type": "notes", "title": "",
     ...
     "author_email": "rep@acme.com",
     "properties": {"hs_note_body": "Security review scheduled; customer wants EU data "
                                    "residency confirmed."},
     "associations": [{"to": "hs-co-acme"}]},
]
...
with serve_or_connect(CORPUS, url=args.url) as mock:
    ...
    for c in companies:
        p = c.properties
        print(f"  - {p.get('name')} [{p.get('lifecyclestage')}] {p.get('domain')} "
    ...
    print(f"\ndeals → {len(deals)}")
    for d in deals:
        print(f"  - {d.properties.get('dealname')} ${d.properties.get('amount')} "
