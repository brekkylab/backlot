# Using official SDKs against Backlot

One runnable, **self-contained** script per service — each spins up its own server (via
...
that at Backlot takes a real shim.
...
an example nobody executes cannot back the claim that Backlot works with the real client — so it
...
npm install && npx tsx index.ts                       # spawns its own server, like the Python ones
...
- **The `Authorization` header carries the BARE key**, with no `Bearer` prefix — that is how a
  Linear personal API key travels. (Backlot accepts `Bearer <token>` too, Linear's OAuth shape.)
...
view — Backlot derives its org from the corpus and 404s any other owner, so nothing hardcodes one),
...
Pass `--url http://host:port` to point a script at an already-running server instead; if it's
...
library's own token exchange runs against Backlot's `POST /oauth2/token` in both:
...
against a server it starts itself, asserting all 39 checks pass across the 6 SDKs:
