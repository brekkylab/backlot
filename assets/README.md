...
| Path | What it is |
|---|---|
| `slack-onboarding/` | The README's demo — a [Remotion](https://remotion.dev) composition, rendered to a GIF |
| `slack-onboarding/demo.gif` | What the README embeds. Generated by `build.sh`, committed |

A second demo is a new directory beside it. Nothing in `slack-onboarding/` is shared
infrastructure — the previous demo's screenplay renderer was, and it turned out that a demo's
hard part is what it says, not how it is encoded.

## Rebuilding

```bash
brew install ffmpeg gifsicle
cd assets/slack-onboarding && npm install && ./build.sh
```

`build.sh` renders the composition to `out/video.mp4`, encodes `demo.gif`, and prints its size.
...
**15fps, not 30.** The old terminal demo needed 30 because `make_cast.py` typed at 50ms per
character and smooth typing needs a frame per character. Nothing here types; the motion is
opacity and a 10px rise, and it reads the same at 15fps for half the frames.
