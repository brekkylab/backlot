...
def test_head_with_two_subresources_is_the_conflicts_400_with_an_empty_body(live_server):
    ...
    for path in (
        ...
        assert err.headers.get("Content-Type") == "application/xml"
        # Real S3 sends no Allow on the conflict either (measured); two selectors at once is
        # never a case Backlot answers on any method, `?location` or `?uploads` included.
