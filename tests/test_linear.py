...
# copied here as literals so the suite runs offline; a literal moves when a later measurement finds
# the vendor has. `IntegrationService` has gained two members that way since that date, and the
# enum in `backlot/graphql/linear.graphql` carries their dates. `backlot diff --source linear` is
# the live re-measurement and `backlot/fidelity/baseline/linear.json` the record of the gaps it
# accepts.
